<template>
	<div>
        ...
        <b-row class="mt-2">
          <b-col cols="12" md="9">
            <page-title icon="fas fa-copy" main="Parcelas a Receber"></page-title> 
          </b-col>
        </b-row>
          ...
          <b-alert class="mt-3 w-75 alert-link" variant="warning" show dismissible>Nenhuma Parcela a Receber encontrada.</b-alert> 
			</template>	
		</div>
	</div>
</template>
...
export default {
  ...
  components: {
	...
	methods: {
      ...
      let response
      try {
        response = await RestConnection.get('parcelas-a-receber/consultar')
