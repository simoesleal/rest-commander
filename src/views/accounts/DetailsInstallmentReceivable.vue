<template>
	<div>
        ...
        <b-row class="mt-2">
          <b-col cols="12" md="9">
            <page-title icon="fas fa-copy" main="Detalhes Parcela a Receber"></page-title> 
          </b-col>
        </b-row>
	...
	methods: {
      ...
      this.$router.push({ name: 'ContasReceber' })
		...
		async alterRecord() {
			...
			try {
				response = await RestConnection.put('parcelas-a-receber/atualizar/parcela/', parameters)
			} catch (exception) {
		...
		async cancelarParcela() {
			...
			try {
				response = await RestConnection.put('parcelas-a-receber/atualizar/parcela/cancelar', parameters)
			} catch (exception) {
          if (exception && exception.response && exception.response.data &&   exception.response.data.mensagem) {
            return alert(exception.response.data.mensagem)
          } else {
