  ...
  <div>
    <template>
      <b-container>
        <b-row class="mt-2">
          <b-col cols="12" md="9">
            <page-title icon="fas fa-weight-hanging" main="Cadastrar Unidade de Medida"></page-title> 
          </b-col>
        </b-row>
...
export default {
  ...
  methods: {
    ...
    async saveRecord() {
      let response
      let parameters = {
        name: this.unit.nomeUnidade,
        abbreviation: this.unit.abreviatura,
        status: true,
      }
      try {
        response = await RestConnection.post('unidade-medida/cadastrar/', parameters)
      } catch (exception) {
          if (exception && exception.response && exception.response.data &&   exception.response.data.mensagem) {
            return alert(exception.response.data.mensagem)
          } else {
            return alert('Não foi Salvar esta Unidade de Medida.')
          }
      }
    ...
    async alterRecord() {
      ...
      try {
        response = await RestConnection.put('unidade-medida/atualizar/', parameters)
      } catch (exception) {
        if (exception && exception.response && exception.response.data &&   exception.response.data.mensagem) {
          return alert(exception.response.data.mensagem)
        } else {
          return alert('Não foi possível Atualizar esta Unidade de Medida.')
        }
      }
      alert(response.data.mensagem)
      this.backOnePage()
    ...
    async deleteRecord() {
      let response
      try {
        response = await RestConnection.delete('unidade-medida/deletar/' + this.unit.id)
      } catch (exception) {
          if (exception && exception.response && exception.response.data &&   exception.response.data.mensagem) {
            return alert(exception.response.data.mensagem)
          } else {
            return alert('Não foi possível Deletar esta Unidade de Medida.')
          }
      }
      alert(response.data.mensagem)
      this.backOnePage()    
